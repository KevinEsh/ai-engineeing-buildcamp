...
This book is intended for a di?erent audience, and it has di?erent goals. I
...
goals is to help them learn C. For that part of the class, I use Gri?ths and Grif-
?ths, Head First C, from O’Reilly Media. This book is meant to complement
that one.
...
Chapter 1 explains some of the di?erences between compiled and interpreted
...
Chapter 4 is about ?le systems and data streams. Recommended reading:
...
text, I have not yet included the ?gures. So there are a few places where, I’m
sure, the explanation will be greatly improved when the ?gures are ready.
...
keep track of the ?les that make up a project. A collection of ?les under
...
(cid:136) Andy Engle knows the di?erence between GB and GiB.
...
Representing ?oating-point numbers . . . . . . . . . . . . . .
...
guage; nevertheless, there are some general di?erences between compiled and
...
di?erent types. Any values that support the addition operator will work; any
...
The ?rst line of the function includes type declarations for the parameters
...
directives that take e?ect before the program is compiled. For example,
the #include directive causes the source code from another ?le to be
...
5. Linking: If the program uses values and functions de?ned in a library,
the compiler has to ?nd the appropriate library and include the required
code.

1This is a simpli?cation; we will go into more detail later.
...
cutable ?le. For example, here is a minimal C program:
...
If you save this code in a ?le called hello.c, you can compile and run it like
this:

$ gcc hello.c
$ ./a.out

By default, gcc stores the executable code in a ?le called a.out (which origi-
nally stood for “assembler output”). The second line runs the executable. The
pre?x ./ tells the shell to look for it in the current directory.

It is usually a good idea to use the -o ?ag to provide a better name for the
executable:
...
The -c ?ag tells gcc to compile the program and generate machine code, but
not to link it or generate an executable:

$ gcc hello.c -c

The result is a ?le named hello.o, where the o stands for object code, which
is the compiled program. Object code is not executable, but it can be linked
into an executable.

The UNIX command nm reads an object ?le and generates information about
the names it de?nes and uses. For example:
...
This output indicates that hello.o de?nes the name main and uses a function
...
You can control how much optimization gcc does with the -O ?ag. By default,
...
than to speed it up. But if your program has a subtle bug, you might ?nd that
optimization makes the bug appear or disappear. It is usually a good idea to
turn o? optimization while you are developing new code. Once the program
is working and passing appropriate tests, you can turn on optimization and
con?rm that the tests still pass.

1.5 Assembly code

Similar to the -c ?ag, the -S ?ag tells gcc to compile the program and generate
assembly code, which is basically a human-readable form of machine code.

$ gcc hello.c -S

The result is a ?le named hello.s, which might look something like this:
...
gcc is usually con?gured to generate code for the machine you are running on,
so for me it generates x86 assembly language, which runs on a wide variety
of processors from Intel, AMD, and others. If you are running on a di?erent
architecture, you might see di?erent code.

1.6 Preprocessing

Taking another step backward through the compilation process, you can use
the -E ?ag to run the preprocessor only:

$ gcc hello.c -E
The result is the output from the preprocessor. In this example, it contains
the included code from stdio.h, and all the ?les included from stdio.h, and
all the ?les included from those ?les, and so on. On my machine, the total is
...
If you use a function that’s not de?ned in any of the standard libraries, you
...
perform another illegal ?oating-point operation, you will get a “Floating point
...
Before we talk about processes, I want to de?ne a few words:

(cid:136) Abstraction: An abstraction is a simpli?ed representation of something
...
a noti?cation when it is available for pickup. I don’t need to know where
...
with many other virtual machines, on a computer running a di?erent operating
system.
...
undesired e?ects on other components.
...
have di?erent data and hardware states.
...
are implemented. But if you are curious, you will ?nd a lot of interesting
...
The ?rst column is the unique numerical process ID. The second column is
...
If you use the -e ?ag, you get every process (including processes belonging to
other users, which is a security ?aw, in my opinion).
...
init is the ?rst process created when the operating system starts. It creates
...
speci?cally, it handles software interrupt requests, or “soft IRQ”.
...
Suppose I ?ip a coin and tell you the outcome. I have given you one bit of
...
is something you were already con?dent of, con?rming it contributes only a
...
If the process reads and writes ?les, those ?les are usually stored on a hard
...
well as RAM, but the properties of these devices are very di?erent, so we will
need to distinguish them. I will use storage to refer to HDDs and SSDs.

3.3 Address spaces

Each byte in main memory is speci?ed by an integer physical address. The
...
1,073,741,823 in decimal, or 0x3?f ?? in hexadecimal (the pre?x 0x indicates
...
the virtual address space runs from 0 to 0x?? ??. The size of this address
...
they would map to di?erent locations in physical memory.
...
The data of a running process is organized into ?ve segments:
...
address of the ?rst machine language instruction in main, which we expect to
...
program (as opposed to a string that is read from a ?le, input by a user, etc.).
...
If you add this function to aspace.c you can con?rm that counter is allocated
...
o?set. A “page” is a chunk of memory; the size of a page depends on
the operating system and the hardware, but common sizes are 1–4 KiB.

3. The MMU looks up the page number in the translation lookaside bu?er
(TLB) and gets the corresponding physical page number. Then it com-
bines the physical page number with the o?set to produce a PA.
...
(cid:136) The size of the o?set is determined by the page size. In this example the

page size is 210 B, so it takes 10 bits to specify a byte on a page.

(cid:136) If a VA is 32 bits and the o?set is 10 bits, the remaining 22 bits make

up the virtual page number.

(cid:136) Since there are 220 physical pages, each physical page number is 20 bits.

Adding in the 10 bit o?set, the resulting PAs are 30 bits.
...
org/wiki/Page_table; you might ?nd the details interesting. But the fun-
...
quentially, but they raise a di?erent complication: each block can be written
...
common abstraction is called a “?le system.”

Abstractly:

(cid:136) A “?le system” is a mapping from each ?le’s name to its contents. If
you think of the names as keys, and the contents as values, a ?le system
is a kind of key-value database (see https://en.wikipedia.org/wiki/
Key-value_database).

(cid:136) A “?le” is a sequence of bytes.

File names are usually strings, and they are usually “hierarchical”; that is, the
string speci?es a path from a top-level directory (or folder), through a series
of subdirectories, to a speci?c ?le.

24

Chapter 4. Files and ?le systems

The primary di?erence between the abstraction and the underlying mechanism
is that ?les are byte-based and persistent storage is block-based. The operating
system translates byte-based ?le operations in the C library into block-based
operations on storage devices. Typical block sizes are 1–8 KiB.

For example, the following code opens a ?le and reads the ?rst byte:
...
1. fopen uses the ?lename to ?nd the top-level directory, called /, the

subdirectory home, and the sub-subdirectory downey.

2. It ?nds the ?le named file.txt and “opens” it for reading, which means
it creates a data structure that represents the ?le being read. Among
other things, this data structure keeps track of how much of the ?le has
been read, called the “?le position”.
...
there seems to be no good name for it. It is an entry in the open ?le
table, so I will call it an OpenFileTableEntry.

3. When we call fgetc, the operating system checks whether the next char-
acter of the ?le is already in memory. If so, it reads the next character,
advances the ?le position, and returns the result.
...
memory, and the process resumes. It reads the ?rst character and stores
it as a local variable.

6. When the process closes the ?le, the operating system completes or can-
cels any pending operations, removes data stored in memory, and frees
the OpenFileTableEntry.

The process for writing a ?le is similar, but there are some additional steps.
Here is an example that opens a ?le for writing and changes the ?rst character.
...
1. Again, fopen uses the ?lename to ?nd the ?le. If it does not already
exist, it creates a new ?le and adds an entry in the parent directory,
/home/downey.

2. The operating system creates an OpenFileTableEntry that indicates that

the ?le is open for writing, and sets the ?le position to 0.

3. fputc attempts to write (or re-write) the ?rst byte of the ?le. If the
?le already exists, the operating system has to load the ?rst block into
memory. Otherwise it allocates a new block in memory and requests a
new block on disk.

4. After the block in memory is modi?ed, it might not be copied back to
the disk right away. In general, data written to a ?le is “bu?ered”, which
means it is stored in memory and only written to disk when there is at
least one block to write.

5. When the ?le is closed, any bu?ered data is written to disk and the

OpenFileTableEntry is freed.

To summarize, the C library provides the abstraction of a ?le system that
maps from ?le names to streams of bytes. This abstraction is built on top of
...
hardware provide several features intended to “?ll in” this gap:
...
if you open a ?le and read the ?rst block, there is a good chance you
will go on to read the second block. The operating system might start
loading additional blocks before they are requested.

(cid:136) Bu?ering: As I mentioned, when you write a ?le, the operating system
...
the problem, and (2) when data is bu?ered, it can be harder to debug a
program. For example, if a program prints a value and then crashes, the value
might not appear, because it might be in a bu?er. Similarly, if a program
...
The blocks that make up a ?le might be arranged contiguously on disk, and ?le
...
In many UNIX ?le systems, that data structure is called an “inode,” which
stands for “index node”. More generally, information about ?les, including
the location of their blocks, is called “metadata”. (The content of the ?le is
data, so information about the ?le is data about data, hence “meta”.)

Since inodes reside on disk along with the rest of the data, they are designed
to ?t neatly into disk blocks. A UNIX inode contains information about a
?le, including the user ID of the ?le owner; permission ?ags indicating who
is allowed to read, write, or execute it; and timestamps that indicate when it
was last modi?ed and accessed. In addition, it contains block numbers for the
?rst 12 blocks that make up the ?le.

If the block size is 8 KiB, the ?rst 12 blocks make up 96 KiB. On most systems,
that’s big enough for a large majority of ?les, but it’s de?nitely not big enough
...
for all but the largest ?les, but still not big enough for all.
...
And if that’s not big enough, there is (?nally) a triple indirection block, which
contains pointers to double indirection blocks, yielding a maximum ?le size of
8 TiB. When UNIX inodes were designed, that seemed big enough to serve for
a long time. But that was a long time ago.

As an alternative to indirection blocks, some ?les systems, like FAT, use a File
Allocation Table that contains one entry for each block, called a “cluster” in
this context. A root directory contains a pointer to the ?rst cluster in each ?le.
The FAT entry for each cluster points to the next cluster in the ?le, similar to
...
File systems have to keep track of which blocks belong to each ?le; they also
have to keep track of which blocks are available for use. When a new ?le is
created, the ?le system ?nds an available block and allocates it. When a ?le
is deleted, the ?le system makes its blocks available for re-allocation.
...
It is hard to design a ?le system that achieves all of these goals, especially
since ?le system performance depends on “workload characteristics” like ?le
sizes, access patterns, etc. A ?le system that is well tuned for one workload
might not perform as well for another.

For this reason, most operating systems support several kinds of ?le systems,
and ?le system design is an active area of research and development. In the
last decade, Linux systems have migrated from ext2, which was a conventional
UNIX ?le system, to ext3, a “journaling” ?le system intended to improve speed
and contiguity, and more recently to ext4, which can handle larger ?les and
?le systems. Within the next few years, there might be another migration to
the B-tree ?le system, Btrfs.

4.4 Everything is a ?le?

The ?le abstraction is really a “stream of bytes” abstraction, which turns out
to be useful for many things, not just ?le systems.
...
as input into another process. For the ?rst process, the pipe behaves like a
?le open for writing, so it can use C library functions like fputs and fprintf.

4.4. Everything is a ?le?

29

For the second process, the pipe behaves like a ?le open for reading, so it uses
fgets and fscanf.
...
processes on di?erent computers (usually). Again, processes can read data
from and write data to a socket using “?le” handling functions.

Reusing the ?le abstraction makes life easier for programmers, since they only
have to learn one API (application program interface). It also makes programs
more versatile, since a program intended to work with ?les can also work with
...
To ?nd the two’s complement of a negative number, ?x, ?nd the binary rep-
resentation of x, ?ip all the bits, and add 1. For example, to represent ?510,
...
positive number and add 1’s for a negative number. In e?ect, we have to copy
...
bers) unless you declare them unsigned. The di?erence, and the reason this
...
set bits, and ^ is used to ?ip, or “toggle” bits. Here are the details:
...
5.3. Representing ?oating-point numbers
...
Toggling bits: Finally, if you XOR a vector with 3, it ?ips the rightmost bits
...
Floating-point numbers are represented using the binary version of scienti?c
notation. In decimal notation, large numbers are written as the product of a
coe?cient and 10 raised to an exponent. For example, the speed of light in
m/s is approximately 2.998 · 108.

Most computers use the IEEE standard for ?oating-point arithmetic. The C
...
are the exponent, q, and the last 23 bits are the coe?cient, c. The value of a
...
In ?oating point,
that’s 1.101 · 23, so the exponent is 3 and the part of the coe?cient that would
...
To pack and unpack ?oating-point numbers in C, we can use a union and
...
The union allows us to store a ?oating-point value using p.f and then read it
...
To get the coe?cient, we need to extract the 23 rightmost bits and ignore the
...
including the bias. And the coe?cient, which I printed in hexadecimal, is 101
...
that might be an integer, ?oat, complex, or rational number.

However, unions are error-prone. It is up to you, as the programmer, to keep
track of what type of data is in the union; if you write a ?oating-point value
...
stack and ?lls it with the numbers from 0 to 99.
...
Next I’ll de?ne a function that creates a smaller array and deliberately accesses
...
In this example, all of the variables are integers, so it is relatively easy to ?gure
...
to make an array of ?oats, the results are:
...
The latter two values are what you get if you interpret a ?oating-point value
as an integer. If you encountered this output while debugging, you would have
a hard time ?guring out what’s going on.
...
A careful observer will notice that they di?er by a single bit. And this pattern
...
case to upper-case by ?ipping the sixth bit. As a challenge, you can make a
...
Conversely, if you write a string and then accidentally read it as an int or ?oat,
...
You will ?nd that the ASCII representation of the ?rst 8 characters
is
of my name,
69779713878800585457664.

interpreted as a double-precision ?oating point number,
...
It might not sound di?cult to follow these rules, but in a large program a chunk
...
in di?erent parts of the program. The chunk should not be freed until all
references to the chunk are no longer in use. Getting this right often requires
careful analysis across all parts of the program, which is di?cult and contrary
...
To make matters worse, memory errors can be di?cult to ?nd because the
...
be di?erent each time the program runs.
...
At that point it will be very di?cult to ?nd the source of the problem.
...
references to the same chunk in di?erent parts of your program.

Often there is a trade-o? between safe memory management and performance.
...
will increase inde?nitely. At that point, a few things might happen:
...
(cid:136) Eventually, a process might ?ll its virtual address space (or the usable
...
error condition. Sometimes other codes are used to indicate di?erent error
conditions.
...
Because memory management is so di?cult, most large programs, like web
...
When malloc is called, it checks whether it can ?nd a free chunk that’s big
...
patterns, it runs fast and uses memory e?ciently.
...
chunks, malloc is not space e?cient.
If your program requires large
numbers of small structures, it might be more e?cient to allocate them
in arrays.
...
the program down by making memory caches less e?ective.
...
(cid:136) A status register, or ?ag register, that contains information about the
current computation. For example, the ?ag register usually contains a
...
Most computers can execute a few hundred di?erent instructions, called the
...
(cid:136) Jump/branch: Changes the program counter, causing the ?ow of execu-
tion to jump to another location in the program. Branches are usually
conditional, which means that they check a ?ag in the ?ag register and
...
The performance of many programs is limited by the e?ectiveness of the cache.
...
The cache “hit rate”, h, is the fraction of memory accesses that ?nd data in
...
Equivalently, we could de?ne the “miss penalty” as the extra time to process
...
When a program reads a byte for the ?rst time, the cache usually loads a
...
64, and the ?rst byte of the string happens to fall at the beginning of a block.
When you load the ?rst byte, you incur a miss penalty, but after that the rest
...
My modi?ed version of this program is in the cache directory of the repository
...
the average time with su?cient precision.
...
The second loop runs the same number of iterations as the ?rst. After each
...
to increment temp. This di?erence is the total miss penalty incurred by all
...
temporal locality. If the entire array ?ts in cache, we expect the average
...
the array does not ?t in cache.

(cid:136) If the stride is equal to the block size (or greater), the spatial locality is
e?ectively zero, because each time we read a block, we only access one
...
write programs that use them more e?ectively.
...
drawback of cache-aware algorithms is that they are hardware-speci?c.
...
The trade-o? between speed, size, and cost is the fundamental reason for
...
a word is 64 bits or 8 B. So the total size of the register ?le is 100–300 B.
...
For the other numbers in the table, I looked at the speci?cations for typical
...
memory when they execute programs and open ?les. But the operating
...
system reads a ?le from disk, it might read 10s or 100s of blocks at a
time.

(cid:136) When does data get moved? In the most basic cache, data gets moved
into cache when it is used for the ?rst time. But many caches use some
...
memory to disk, then it modi?es the page table for Process B to indicate
...
event that stops the normal instruction cycle and causes the ?ow of execution
...
the condition can be handled by the operating system. Some ?oating-point
...
interrupt, causing the ?ow of execution to jump to the kernel. The kernel
...
the program counter of the interrupted process, which has the e?ect of
...
The interrupt handler ?gures out which process was waiting for the re-
...
(cid:136) Processes might be limited by di?erent resources. A process that does
...
20 minutes to prepare the ?lling. If you prepare the crust ?rst, you can
prepare the ?lling while the crust is chilling, and you can ?nish the pie in
35 minutes. If you prepare the ?lling ?rst, the process takes 55 minutes.
...
a substantial e?ect on performance. Simple scheduling policies are usually
good enough.
...
applications, a general-purpose operating system like Linux can be modi?ed
to handle real-time scheduling. These modi?cations might include:
...
ority runs within a ?xed amount of time.
...
the same code. But di?erent threads often run di?erent parts of the code.
...
Pthreads for short. The POSIX standard de?nes a thread model and an
interface for creating and controlling threads. Most versions of UNIX provide
an implementation of Pthreads.

Using Pthreads is like using most C libraries:

(cid:136) You include headers ?les at the beginning of your program.

(cid:136) You write code that calls functions de?ned by Pthreads.
...
The ?rst two are standard; the third is for Pthreads and the fourth is for
semaphores. To compile with the Pthread library in gcc, you can use the -l
option on the command line:

gcc -g -O2 -o array array.c -lpthread

This compiles a source ?le named array.c with debugging info and opti-
...
ond, Shared is a structure I de?ned to contain values shared between threads.
...
The ?rst parameter is a pointer to a function that takes a void pointer and
...
?nd this example in counter/counter.c. You can compile and run it like
this:
...
When you run it, you will probably get di?erent results. And if you run it
again, you might get di?erent results each time. What’s going on?
...
Each time you run the program, threads might be interrupted at di?erent
points, or the scheduler might choose di?erent threads to run, so the sequence
of events, and the results, will be di?erent.

3

Suppose we want to impose some order. For example, we might want each
thread to read a di?erent value of counter and increment it, so that the value
of counter re?ects the number of threads that have executed child_code.
...
I’ll show you how to use it ?rst; then I’ll explain how it works.
...
has the e?ect of barring all other threads. Suppose Thread A has locked the
...
proceed. In e?ect, the threads line up to execute child_code one at a time,
...
And that satis?es the requirements. In order for this solution to work, I have
...
The code in this section is in counter_mutex.c. The de?nition of Mutex is in
mutex.c, which I explain in the next section.

9.5 Mutex

My de?nition of Mutex is a wrapper for a type called pthread_mutex_t, which
is de?ned in the POSIX threads API.
...
My code makes it easier to get that right. It de?nes a type, Mutex, which is
...
Then it de?nes make_mutex, which allocates space and initializes the mutex:
...
This code is in mutex.c and the header ?le mutex.h.
...
In some multi-threaded programs, threads are organized to perform di?erent
...
wrong and ?x it. The code for this example is in the repository for this book,
in a folder called queue. The ?le queue.c contains a basic implementation of
a circular bu?er, which you can read about at https://en.wikipedia.org/
...
Here’s the structure de?nition:
...
make_queue allocates space for this structure and initializes the ?elds:
...
To avoid that, we de?ne another special case to indicate that the queue is full:
...
And ?nally here’s the shared structure that contains the queue:
...
(cid:136) Access to the queue is not thread safe. Di?erent threads could access
array, next_in, and next_out at the same time and leave the queue in
a broken, “inconsistent” state.

(cid:136) If the consumer is scheduled ?rst, it ?nds the queue empty, print an error
...
In the next section, we solve the ?rst problem with a Mutex. In the following
...
is required to lock the mutex ?rst; this requirement is part of the documented
...
I’ll handle the ?rst condition here, and you will have a chance to handle the
...
Now in queue_pop, if we ?nd the queue empty, we don’t exit; instead we use
...
cond_wait is complicated, so let’s take it slow. The ?rst argument is the
...
there are no threads waiting on the condition variable, the signal has no e?ect.
...
In the next section I’ll show you how my Cond code works, but ?rst I want to
...
called pthread_cond_t, which is de?ned in the POSIX threads API. It is very
similar to Mutex, which is a wrapper for pthread_mutex_t. Both wrappers
are de?ned in utils.c and utils.h.
...
making it easier to work with. And it presents a ?nal challenge: can you write
...
The POSIX standard speci?es an interface for semaphores; it is not part of
...
to make it easier to use. The interface is de?ned in sem.h:
...
Semaphore is a synonym for sem_t, but I ?nd it more readable, and the capital
...
Here’s the new de?nition of Queue, replacing the mutex and condition variables
with semaphores:
...
ables and mutexes. In the repository for this book, you’ll ?nd my solution in
mysem_soln.c and mysem_soln.h.

If you have trouble getting started, you can use the following structure de?ni-
...
do...while loop. Can you ?gure out why it is not a more conventional while
